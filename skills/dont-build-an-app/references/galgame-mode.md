...
顶栏显示当前角色章节（例如「第二章：jobs」）；设置使用齿轮贴图，对话框底部居中放回看、存档、设置、加速四个图标。左右箭头只用于回放已经读过的台词：左箭头回看上一句，右箭头仅在回看期间出现，回到实际进度后消失。回看不会修改存档进度、提交回答、恢复角色、重新生成或启动开发；草稿保留，新回复不会强行打断回看。返回当前进度后恢复正常交互。这些行为由网页负责，Agent 不需重新输出历史台词。
