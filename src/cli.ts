...
import {QwenWorkAdapter} from './adapters/qwenwork.ts';

async function main() {
  if (process.argv[2] !== 'probe') {
    const context=hostContext();
    const adapter=context.host==='workbuddy'?new WorkBuddyAdapter(context.threadId,context.origin!):context.host==='qwenwork'?new QwenWorkAdapter(context.threadId):undefined;
    ...
    if(adapter){
      ...
      if(!loaded){adapter.close();throw Error('宿主原任务历史未加载完成');}
    }
