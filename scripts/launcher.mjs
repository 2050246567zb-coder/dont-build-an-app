...
const dataDir=resolve(process.env.GALGAME_DATA_DIR || join(root,'.galgame'));
...
async function main(){
  ...
  if(existing){
    ...
    console.log(existing.runtime.url);return;
  }
  await mkdir(dataDir,{recursive:true});
  const out=await open(join(dataDir,'server.stdout.log'),'a'),err=await open(join(dataDir,'server.stderr.log'),'a');
  const child=spawn(process.execPath,[join(root,'dist/cli.js')],{cwd:root,env:{...process.env,GALGAME_DATA_DIR:dataDir},detached:true,windowsHide:true,stdio:['ignore',out.fd,err.fd]});
  ...
  for(let attempt=0;attempt<40;attempt++){
    ...
    if(result?.runtime.pid===child.pid){console.log(result.runtime.url);return;}
  }
