...
输入/交付时使用左人物、右面板的布局。经用户反馈修正后，乔布斯和小黑内置立绘保持相同的 2:3 画布与舞台相对高度，输入出现/收起只横向移动，不再压缩进左侧小矩形；原图大腿处的截断落在对话框内部，不在场景中悬空露出。窄于 550px 时，面部保留在上方，回复面板下移，允许滚动；系统精灵和任意比例的实时生成图继续使用适合自身的完整图片布局。面板自身有高度上限并可滚动，长文本/异常提示不盖住底部台词。最新检查见 [人物比例修复](reports/2026-09-17-portrait-scale.md)。
